			...
			sections: {
				concepts: {
					...
					items: conceptItems.map((ci) => ({
						...
						...(ci.code !== null && { code: ci.code }),
						...(ci.code_language !== null && { code_language: ci.code_language }),
					})),
					...(conceptExamples.length > 0 && {
						examples: conceptExamples.map((ce) => ({
							...
							label: ce.label,
							latex: ce.latex,
							...(ce.code !== null && { code: ce.code }),
							...(ce.code_language !== null && { code_language: ce.code_language }),
						})),
					}),
				},
				applications: {
					...
					items: applicationItems.map((ai) => ({
						...
						...(ai.code !== null && { code: ai.code }),
						...(ai.code_language !== null && { code_language: ai.code_language }),
					})),
				},
			},
			footer: {
